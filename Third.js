// There are N blocks, numbered from 0 to N-1, arranged in a row. A couple of frogs were sitting together on one block when they had a 
//terrible quarrel. Now they want to jump away from one another so that the distance between them will be as large as possible.
...
//and the second block is of the same or greater height as the first. What is the longest distance that they can possibly 
...
function calculateDistance(blocks, start){
    let distance = 0;
    let J = start;
    let K;
...
calculateDistance([2, 6, 8, 5], 0); // returns 3
calculateDistance([1, 5, 5, 2, 6], 3);// returns 4
calculateDistance([1, 1], 1);// returns 2