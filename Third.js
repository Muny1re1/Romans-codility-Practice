...
// "-": the machine pops the two topmost elements from the stack, 
//subtracts the second one from the first (topmost) one and pushes the difference onto the stack.
// After processing all the operations, the machine returns the topmost value from the stack.

// The machine processes 20-bit unsigned integers (numbers from 0 to 220 − 1).
...
// Given a string "5 6 + -", the machine reports an error. 
//After the addition, there is only one number on the stack and the subtraction operation expects two.
...
// that, given a string S containing a sequence of operations for the word machine, 
...
function algorithm(S){
    // initializing the stack, operations, max value and result
    let stack = [];
    ...
    // iterating over each operation in the string and updating the result accordingly
    for (let operation of operations){
        if(operation === " "){
            continue;
        } else {
            if(Number.isInteger(parseInt(operation))){
                ...
                switch(operation){
                    ...
                    case "+":
                        ...
                        let summation = stack.pop() + stack.pop();
                        if(summation > MAX){
                           result = -1
                           console.log(result);
                           return result
                        }
                        stack.push(summation);
                        break;
                    case "-":
                        if(stack.length < 2) {
                            result = -1
                        ...
                        let subtraction  = stack.pop() - stack.pop();
                        if (subtraction < 0) {
                            result = -1
                            console.log(result);
                            return result;
